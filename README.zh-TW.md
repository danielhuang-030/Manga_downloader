...
- **停止下載**：成功「開始下載」後，同一顆主要按鈕會變成「停止」；按下後為**合作式取消**，按鈕顯示「停止中…」，直到後端執行緒結束並釋放 WebDriver。在此之前無法再成功啟動新任務（進行中或收尾時再按「開始下載」會得到 HTTP **409**）。由使用者停止時，SSE 會以 `type: "run_cancelled"` 結束串流。**暫停／繼續**尚未實作。
